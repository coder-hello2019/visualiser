...
class Node:
    ...
    def getNeighbours(self, board):
        ...
        # get left neigh.
        if self.col >= 0 and not board[x - 1][y].isWall():
            self.neighbours.append(board[x - 1][y])
        # get up neigh.
        if self.row > 0 and not board[x][y - 1].isWall():
            self.neighbours.append(board[x][y - 1])
        # get down neigh.
        if self.row < ROWS - 1 and not board[x][y + 1].isWall():
             self.neighbours.append(board[x][y + 1])
        # get right neigh.
        if self.col < COLS - 1 and not board[x + 1][y].isWall():
            self.neighbours.append(board[x + 1][y])
